Write-Output '5.6 (L1) Host should reject forged transmits on standard virtual switches and port groups (Automated)'
...
if($command.Count -gt 0){
    ...
    foreach($switchs in $command){
        $vm = $switchs.VMHost
        $mac_change = $switchs.ForgedTransmits
    ...
    if($needRemediation){
        $command = esxcli network vswitch standard policy security set -v vSwitch2 -f false
