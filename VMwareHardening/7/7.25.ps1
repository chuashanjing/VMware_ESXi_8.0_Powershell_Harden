Write-Output '7.25 (L1) Virtual machines must limit the number of retained diagnostic logs (Automated)'
...
$result = Get-VM | Get-AdvancedSetting -Name "log.keepOld" | Select Entity, Name, Value
$result
...
if($result.Count -gt 0){
    ...
    if($needsRemediation) {
        ...
        $command = Get-VM | New-AdvancedSetting -Name "log.keepOld" -value "10"
        Write-Output "Rerun script to view changes"    
    }
    else{
        Write-Output 'All VMs are compliant. 7.25 is compliant'
    }
}
else{
