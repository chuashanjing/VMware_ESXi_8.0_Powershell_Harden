Write-Output '7.13 (L1) Virtual Machines must remove unnecessary parallel port devices (Automated)'
    ...
    Process {   
        Foreach ($VMachine in $VM) {  
            Foreach ($Device in $VMachine.ExtensionData.Config.Hardware.Device) {  
                ...
                $Details | Add-Member Noteproperty VMName -Value $VMachine.Name   
...
# Retrieve all VMs and their Parallel Port details
$result = Get-VM | Get-ParallelPort 
$result 
...
if($result.Count -gt 0){   
    ...
    if($needsRemediation) {  
        ...
        $command = Get-VM | Get-ParallelPort | Remove-ParallelPort
        Write-Output 'Rerun script to view changes'        
    }   
    else{   
        Write-Output 'All VMs are compliant. 7.13 is compliant'   
    } 
} 
else{   
