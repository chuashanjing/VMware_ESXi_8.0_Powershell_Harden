Write-Output '7.15 (L1) Virtual Machines must remove unnecessary CD/DVD devices (Automated)'
...
if($result.Count -gt 0){
    ...
    if($needsRemediation) {
        ...
        $command = Get-VM | Get-CDDrive | Remove-CDDrive
        Write-Output "Rerun script to view changes"    
    }
    else{
        Write-Output 'All VMs are compliant. 7.14 is compliant'
    }
}
