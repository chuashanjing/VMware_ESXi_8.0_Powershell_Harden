Write-Output '7.20 (L1) Virtual Machines must deactivate virtual disk shrinking operations (Automated)'
...
$result = Get-VM | Get-AdvancedSetting -Name "isolation.tools.diskShrink.disable" | Select Entity, Name, Value
$result
...
if($result.Count -gt 0){
    ...
    if($needsRemediation) {
        ...
        $command = Get-VM | New-AdvancedSetting -Name "isolation.tools.diskShrink.disable" -value $true
        Write-Output "Rerun script to view changes"    
    }
    else{
        Write-Output 'All VMs are compliant. 7.20 is compliant'
    }
}
else{
