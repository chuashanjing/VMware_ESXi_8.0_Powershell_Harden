Write-Output '7.8 (L1) Virtual Machines must prevent unauthorized modification of devices (Automated)'
...
$result = Get-VM | Get-AdvancedSetting -Name "isolation.device.connectable.disable" | Select Entity, Name, Value
$result
...
if($result.Count -gt 0){
    ...
    if($needsRemediation) {
        ...
        $command = Get-VM | New-AdvancedSetting -Name "isolation.device.connectable.disable" -value $true
        Write-Output "Rerun script to view changes"    
    }
    else{
        Write-Output 'All VMs are compliant. 7.8 is compliant'
    }
}
else{
