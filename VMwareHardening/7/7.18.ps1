Write-Output '7.18 (L1) Virtual Machines must deactivate console paste operations (Automated)'
...
$result = Get-VM | Get-AdvancedSetting -Name "isolation.tools.paste.disable" | Select Entity, Name, Value
$result
...
if($result.Count -gt 0){
    ...
    if($needsRemediation) {
        ...
        $command = Get-VM | New-AdvancedSetting -Name "isolation.tools.paste.disable" -value $true
        Write-Output "Rerun script to view changes"    
    }
    else{
        Write-Output 'All VMs are compliant. 7.18 is compliant'
    }
}
else{
