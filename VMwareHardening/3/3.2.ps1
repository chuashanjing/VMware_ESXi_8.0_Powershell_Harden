Write-Output "3.2 (L1) Host must deactivate the ESXi shell (Automated)"
...
$command = Get-VMHost | Get-VMHostService | Where { $_.key -eq "TSM" } | Select VMHost, Key, Label, Policy, Running, Required
$command
...
if($command.Count -gt 0){
    ...
    if($needRemediation){
        $command = Get-VMHost | Get-VMHostService | Where { $_.key -eq "TSM" } | Set-VMHostService -Policy Off
